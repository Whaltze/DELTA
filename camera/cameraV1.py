    ...
    # --- 修改处：筛选最优结果 ---
        ...
        for trk in self.trackers:
            if trk['confidence'] >= self.CONFIDENCE_THRESHOLD:
                # 绘制轮廓（所有追踪到的都画，方便调试看到）
                cv2.drawContours(frame, [trk['contour']], -1, (0, 255, 0), 2)
                
                # 2. 只有颜色和形状完全匹配设定的目标，才加入候选列表
                # 如果没有设定目标（None），则全部通过(或者你可以决定默认只通过红正方形)
        ...
        # 3. 如果有候选者，选出置信度最高的“那一个” (Most Precise One)
        if valid_candidates:
            # 按置信度从大到小排序
            ...
            # 绘制特殊标记（红色圆点表示这是选中的目标）
            cX, cY = best_target['center']
            label = f"TARGET: {best_target['color']} {best_target['shape']}"
            cv2.putText(frame, label, (cX - 20, cY - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
            ...
            self.object_detected_robot_coords.emit({
                ...
                'pixel_coords': (cX, cY),
                'confidence': best_target['confidence'] # 附带置信度
            })
